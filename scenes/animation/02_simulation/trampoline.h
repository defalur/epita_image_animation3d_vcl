...
struct gui_scene_structure
{
    ...
    float time_interval_new_sphere = 1.f;
};
...
struct scene_model : scene_base
{
    ...
    void display(std::map<std::string,GLuint>& shaders,
                 scene_structure& scene,
                 gui_structure& gui);
    void display_particles(scene_structure& scene);
    void display_elements(std::map<std::string,GLuint>& shaders,
            scene_structure& scene,
            gui_structure& gui);
    ...
    vcl::buffer2D<vcl::vec3> force_collision;
